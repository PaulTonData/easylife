...
class PostingCreateView(generic.edit.CreateView):
    ...
    def get_success_url(self):
        return reverse('marketplace:detail', kwargs={
            'pk': self.object.pk
        })
...
def send_message(request, posting_id):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('marketplace:send_message_confirm'))
    return render(request, 'marketplace/send_message.html', {'form': form, 'posting_id': posting_id})

def send_message_confirm(request):
    return render(request, 'marketplace/send_message_confirm.html')
